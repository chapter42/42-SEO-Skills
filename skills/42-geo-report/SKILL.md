...
name: 42-geo-report
description: >
  Unified GEO reporting with narrative report, delta comparison, and PDF generation.
  Consolidates audit findings into client-ready deliverables. Modes: --full (narrative),
  --delta (month-over-month), --pdf (charts/gauges), --executive (1-page summary).
  Use when user says "report", "GEO report", "client report", "progress report",
  "monthly report", "PDF report".
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, WebFetch
version: 2.0.0
tags: [report, geo, client, pdf, delta]
---

# 42 GEO Unified Report Generator

Callable as: `/42:geo-report`

## Modes

| Mode | Command | Output | Length |
|---|---|---|---|
| Full narrative | `/42:geo-report --full <domain>` | `GEO-CLIENT-REPORT.md` | 3,000-6,000 words |
| Delta comparison | `/42:geo-report --delta <domain>` | `GEO-MONTHLY-<domain>-<YYYY-MM>.md` | 1,500-2,500 words |
| PDF with charts | `/42:geo-report --pdf <domain>` | `GEO-REPORT-<brand>.pdf` | Visual PDF |
| Executive summary | `/42:geo-report --executive <domain>` | `GEO-EXECUTIVE-<domain>.md` | ~1 page (400-600 words) |

If no mode flag is provided, default to `--full`.

---

## Purpose

This skill aggregates outputs from all GEO audit skills into professional, client-ready deliverables. Reports are written for **business owners and marketing leaders**, not developers -- technical findings are translated into business impact and clear action items with priority levels.

---

## GEO Readiness Score Calculation

All modes share the same composite scoring methodology.
...
| Brand Authority & Entity Presence | 15% | /42:brand-intelligence |
...
All modes require completed audit data. The following skills should have been run first (or their output data must already exist):

| Skill | Output File | Required By |
|---|---|---|
| `/42:audit` | GEO-AUDIT-REPORT.md | --full, --pdf, --executive |
| `/42:content` | GEO-CONTENT-ANALYSIS.md | --full, --pdf, --executive |
| `/42:technical` | GEO-TECHNICAL-AUDIT.md | --full, --pdf, --executive |
| `/42:structured-data` | GEO-SCHEMA-REPORT.md | --full, --pdf, --executive |
| `/42:crawlers` | GEO-CRAWLER-REPORT.md | --full, --pdf, --executive |
| `/42:brand-intelligence` | GEO-BRAND-INTELLIGENCE.md | --full, --pdf, --executive |
| Baseline audit + current audit | Two dated audit files | --delta |

If audit data is missing, instruct the user to run `/42:audit <domain>` first.

---

# MODE: --full

## Complete GEO Progress Report (3,000-6,000 words)

Generate **GEO-CLIENT-REPORT.md** -- a comprehensive, print-ready markdown report with the following 12-section structure. The report must be self-contained, ready to send to a client without editing.
...
Add a brief paragraph: "These scores reflect how likely your content is to be cited by each AI search platform. A score below 50 indicates significant barriers to citation on that platform."
...
- **Content assessment**: E-E-A-T framework per Google's Quality Rater Guidelines
...
The single most powerful retention tool for a GEO agency: show clients **exactly** what improved since they started working with you. Every point gained on the GEO score is proof of value.

### Delta Input

```
/42:geo-report --delta <domain>
/42:geo-report --delta <baseline-file> <current-file>
/42:geo-report --delta example.com --month march-2026
```
...
5. If no files exist: suggest running `/42:audit <domain>` first
...
- Trend = arrow indicator (improved, declined, unchanged)
...
A decline is not necessarily bad -- it can mean new issues were discovered in a deeper audit that were not visible before. Frame declines as "newly discovered opportunities."

### Step 4: Generate Delta Report

Output to `~/.geo-prospects/reports/<domain>-monthly-<YYYY-MM>.md`
...
OVERALL GEO SCORE
  ...
  Baseline   [progress bar]  XX/100  (Label)
  Current    [progress bar]  XX/100  (Label)
  Change     +/- X points (+/-X%)

  Target:    XX/100 by Month 6 (on track / at risk)
```
...
|----------|---------|---------|--------|-------|
| AI Citability & Visibility | [X]/100 | [X]/100 | [+/-X] | [trend] |
| Brand Authority Signals | [X]/100 | [X]/100 | [+/-X] | [trend] |
| Content Quality & E-E-A-T | [X]/100 | [X]/100 | [+/-X] | [trend] |
| Technical Foundations | [X]/100 | [X]/100 | [+/-X] | [trend] |
| Structured Data | [X]/100 | [X]/100 | [+/-X] | [trend] |
| Platform Optimization | [X]/100 | [X]/100 | [+/-X] | [trend] |
| **TOTAL** | **[X]/100** | **[X]/100** | **[+/-X]** | **[trend]** |
...
|---|--------|---------|--------|--------|
| 1 | [Action] | [Owner] | Done/In Progress/Not started | [Result] |
...
[Celebrate tangible results -- clients need to see the value clearly.]

---

## New Issues Discovered

[Issues found in current audit that were not in baseline. Frame as opportunities.]
...
| Month 1 | [Date] | [Score] | [+X] | [Achievement] |
| Month 2 | [Date] | [Score] | [+X] | *Current month* |
| Month 3 | -- | -- | -- | [Planned focus] |
| Month 4 | -- | -- | -- | [Planned focus] |
| Month 5 | -- | -- | -- | [Planned focus] |
...
- **Crawlers with access:** [X]/14 -> [Y]/14
...
*Note: Full traffic impact from GEO changes typically takes 4-8 weeks to materialize as AI platforms re-index and update their knowledge bases.*
```

### Delta Output Confirmation

After generating the report, print:
```
Report generated: ~/.geo-prospects/reports/<domain>-monthly-<YYYY-MM>.md

SUMMARY:
GEO Score: XX -> YY (+/-Z points) [trend]
Quick wins completed: X/Y (Z%)
New issues found: X (severity)
On track for Month 6 target: YES/NO (target/100)
```

Suggest next action: "Share with client or run `/42:geo-report --pdf <domain>` for a visual version."

---

# MODE: --pdf

## Professional PDF with Charts and Gauges

Generate a polished, visually designed PDF report using ReportLab. The PDF includes score gauges, bar charts, platform readiness visualizations, color-coded tables, and a prioritized action plan.
...
1. **Check for existing audit data** -- Look for recent GEO audit reports in the current directory or `~/.geo-prospects/`:
   - `GEO-CLIENT-REPORT.md`
   - `GEO-AUDIT-REPORT.md`
   - Any `GEO-*.md` files from a recent audit

2. **If no audit data exists** -- Tell the user to run `/42:audit <domain>` first.
...
4. **Build the JSON** -- Structure all data into the PDF data schema:
...
```json
{
    ...
    "quick_wins": ["Action item 1", "Action item 2"],
    "medium_term": ["Action item 1", "Action item 2"],
    "strategic": ["Action item 1", "Action item 2"],
    "crawler_access": {
...
6. **Run the PDF generator**:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py /tmp/geo-audit-data.json "GEO-REPORT-[brand_name].pdf"
   ```
...
- **GEO Score**: "GEO Score: XX/100" or "Overall: XX/100" or "GEO Readiness Score: XX"
- **Category Scores**: Score tables with columns like "Component | Score | Weight"
- **Platform Scores**: Tables with "Google AI Overviews", "ChatGPT", "Perplexity", etc.
- **Crawler Status**: Tables with "Allowed" or "Blocked" status for GPTBot, ClaudeBot, etc.
- **Findings**: Sections titled "Key Findings", "Critical Issues", "Recommendations"
- **Action Items**: Sections titled "Quick Wins", "Action Plan", "Recommendations"
...
If the user runs `/42:geo-report --pdf https://example.com`:
1. First run a full audit: invoke `/42:audit` for that URL
2. Collect all audit data from the generated report files
...
## Management Summary (1 Page, 400-600 Words)

Generate **GEO-EXECUTIVE-<domain>.md** -- a one-page management summary designed for C-level stakeholders who need the bottom line without the detail.

### Executive Report Structure

```markdown
# GEO Executive Summary: [Brand Name]
**[Domain] | [Date] | Prepared by [Agency]**
...
[One paragraph -- the single most important insight. What is the current state
of AI visibility and what does it mean for the business?]

## Top 3 Priorities

1. **[Action]** -- [Expected impact in one sentence] -- [Effort: X hours/days]
2. **[Action]** -- [Expected impact in one sentence] -- [Effort: X hours/days]
3. **[Action]** -- [Expected impact in one sentence] -- [Effort: X hours/days]

## Business Impact

[One paragraph connecting GEO improvements to revenue. Include:
- Estimated traffic increase from AI search
- Dollar value at current conversion rates
- Competitive context (what competitors are doing better/worse)
- Timeline for results]

## Recommendation

[One paragraph: What should the organization do next? Be specific and direct.
Frame as investment vs. return.]
```

### Executive Mode Guidelines

- **Maximum length**: 1 printed page (400-600 words)
- **Audience**: C-level executives, board members, non-technical stakeholders
- **Tone**: Strategic, business-focused, zero jargon
- **Numbers**: Lead with the score and dollar figures
- **No tables longer than 5 rows** -- keep it scannable
- **End with a clear call to action**

Extract all data from the same audit sources as --full mode, but distill ruthlessly. Every sentence must earn its place on the page.

---

# Formatting and Tone Guidelines (All Modes)

### Formatting
- Use clean markdown: tables, headers (H2/H3), bullet points, bold for emphasis
...
| Skill | Purpose |
|---|---|
| `/42:audit` | Full website GEO+SEO audit -- run first to generate data for all report modes |
...
| `/42:brand-intelligence` | Brand entity presence and authority signals |
