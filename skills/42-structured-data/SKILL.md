...
description: >
  Unified Schema.org audit, validation, scoring and generation for SEO rich results
  AND GEO entity recognition. Detects existing schema, validates correctness, scores
  completeness, generates missing JSON-LD with @graph pattern. Dual purpose: Google
  rich results + AI entity identity (sameAs, speakable, knowsAbout).
  Use when user says "schema", "structured data", "JSON-LD", "rich results",
  "sameAs", "entity", "schema markup".
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, WebFetch
version: 2.0.0
tags: [schema, structured-data, json-ld, seo, geo, entity]
---
...
## Part 3: Schema Type Status (as of April 2026)

> Full reference: `${CLAUDE_PLUGIN_ROOT}/skills/references/schema-types.md` — active types, deprecated, restricted, recent additions.
...
- **FAQPage**: ONLY earns Google rich results for government and healthcare authority sites (restricted Aug 2023). **However**, FAQPage schema still benefits AI/LLM citation visibility (ChatGPT, Perplexity, Google AI Overviews) — AI platforms parse FAQ structured data for question-answer extraction. On commercial sites, flag as **Info priority, not Critical**. Removal removes GEO citation upside.

### DEPRECATED — never recommend

| Schema | Deprecated | Note |
|--------|-----------|------|
| HowTo | Sep 2023 | Fully deprecated. Rich results removed. **Never recommend.** |
...
> Google restricts FAQ rich results to gov/health sites since Aug 2023. Still valuable for AI/LLM citation extraction. On commercial sites, flag as **Info** priority, not Critical — removal removes GEO citation upside.
...
**@graph pattern — entity connection chain:**

The `@graph` array connects entities via `@id` references. The canonical chain is:

```
Organization (@id: /#organization)
  └─ WebSite (@id: /#website) ← publisher ref
       └─ WebPage (@id: /page#webpage) ← isPartOf ref
            └─ Article or Product (@id: /page#article) ← mainEntityOfPage ref
```

Each node references its parent via `@id`, creating a fully connected entity graph that both Google and AI systems can traverse. Always use this chain pattern when generating schema for any page.
...
> Starting-point templates available in `${CLAUDE_PLUGIN_ROOT}/skills/references/schema-templates/`.
...
- **Active/deprecated/restricted types**: `${CLAUDE_PLUGIN_ROOT}/skills/references/schema-types.md`
- **JSON-LD starter templates**: `${CLAUDE_PLUGIN_ROOT}/skills/references/schema-templates/` (organization.json, article-author.json, local-business.json, product-ecommerce.json, software-saas.json, website-searchaction.json)
