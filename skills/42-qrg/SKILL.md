...
For E-E-A-T scoring criteria and the detailed framework used in this assessment, see `${CLAUDE_PLUGIN_ROOT}/skills/references/eeat-framework.md`.
...
Use the DataForSEO **Ranked Keywords** endpoint to pull the top 25 organic keywords for the domain.
...
**Location codes**: 2528 = Netherlands, 2276 = Germany, 2056 = Belgium, 2826 = UK, 2840 = US. Use the dominant market's code for the site.

**Response parsing**: From `tasks[0].result[0].items[]`, extract for each keyword:
- `keyword_data.keyword` -- the keyword text
- `keyword_data.keyword_info.search_volume` -- monthly search volume
- `keyword_data.keyword_info.cpc` -- cost per click
- `keyword_data.keyword_info.competition` -- competition level (0-1)
- `ranked_serp_element.serp_item.rank_group` -- current ranking position
- `keyword_data.keyword_info.keyword_difficulty` -- difficulty score (0-100)

**Cost**: ~$0.05 per call.

Build a table of the top 25: keyword | volume | position | difficulty.
...
From the 25 keywords, select the 5 that are most strategically central to the page's brand positioning -- not just highest volume, but highest relevance to what the page is actually about and trying to be known for.

For each of the 5 brand-core keywords, fetch the current SERP top 5.
...
**Response parsing**: From `tasks[0].result[0].items[]` where `type == "organic"`:
- `domain` -- competitor domain
- `url` -- ranking URL
- `rank_group` -- SERP position
- `title` -- page title
- `description` -- SERP snippet

**Cost**: ~$0.002 per keyword.

For each competitor domain in the SERP results, get Domain Authority.
...
**Response parsing**: From `tasks[0].result[0].items[0]`:
- `rank` -- domain rank
...
Also use `web_fetch` to briefly inspect the top 1-2 competitor pages per keyword: note their content depth, structure, trust signals, and E-E-A-T indicators.
...
For the detailed E-E-A-T scoring rubric and criteria definitions, see `${CLAUDE_PLUGIN_ROOT}/skills/references/eeat-framework.md`.
...
- **Needs Met does not equal Page Quality** -- a great page can still Fail to Meet
...
For the full E-E-A-T framework and scoring rubric, see `${CLAUDE_PLUGIN_ROOT}/skills/references/eeat-framework.md`.
