...
description: >
  Unified brand authority and sentiment analysis. Scans Reddit, YouTube, Wikipedia,
  LinkedIn, news, forums, and reviews for brand mentions. Scores mention volume,
  sentiment distribution, and composite Brand Authority Score. Combines what was
  previously two separate skills (brand-mentions + sentiment).
  Use when user says "brand mentions", "brand authority", "sentiment", "brand analysis",
  "what do people say about us", "online reputation".
...
**YouTube Check:**
1. WebSearch: `[brand name] site:youtube.com`
2. Check: `youtube.com/@[brand-name]` or `youtube.com/c/[brand-name]` for official channel
3. WebSearch: `"[brand name]" site:youtube.com` (exact match for mentions in descriptions)
4. Note: Channel subscriber count, video count, latest upload date, third-party mention count
5. Capture sentiment signals from video titles, descriptions, and comments

**Reddit Check:**
1. WebSearch: `[brand name] site:reddit.com`
2. WebSearch: `"[brand name]" site:reddit.com` (exact match)
3. Check: `reddit.com/r/[brand-name]` for official subreddit
4. Check: `reddit.com/user/[brand-name]` for official account
5. Note: Thread count, dominant subreddits, recommendation frequency
6. Capture full comment content for sentiment analysis (Reddit is the richest sentiment source)
...
**LinkedIn Check:**
1. WebSearch: `[brand name] site:linkedin.com`
...
**News Check:**
1. WebSearch: `"[brand name]"` with time filter for last 6 months
2. WebSearch: `[brand name] site:news.ycombinator.com` (Hacker News)
3. Capture headlines and snippets for sentiment analysis

**Group 3: Forums + Reviews + Other Platforms**

1. WebSearch: `[brand name] site:quora.com`
2. WebSearch: `[brand name] site:stackoverflow.com` (if technical brand)
3. WebSearch: `[brand name] site:github.com` (if technical brand)
4. WebSearch: `[brand name] site:trustpilot.com`
5. WebSearch for topic-relevant review sites (G2, Capterra for SaaS; Yelp for local; Amazon for consumer products)
6. Note: Presence/absence and quality of mentions on each platform
7. Capture review content for sentiment analysis
...
1. **Score sentiment per source** -- Group items by source. For each source group, assess overall positive/negative/neutral percentages.
2. **Extract topics** -- Identify 8-15 specific, concrete topics mentioned across all sources.
3. **Group into themes** -- Organize topics into 3-5 broader themes.
...
5. **Build emotion profile** -- 5 core emotions + 2-3 dynamic context-specific, all summing to 100%.
6. **Calculate overall sentiment** -- Weighted aggregation across all sources with confidence multipliers.

### Step 4: Authority Score Calculation

1. Score each platform (0-100) using the rubrics defined above.
